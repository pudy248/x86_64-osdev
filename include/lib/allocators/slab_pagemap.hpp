...
public:
	...
	slab_pagemap(void* page_ptr)
		: pages((slab_allocator<1, 4096>*)page_ptr, (slab_allocator<1, 4096>*)((uint64_t)page_ptr + 4096 * PC)) {
	...
	uint64_t mem_used() {
		...
		for (int index = 0; index < allocated_pages; index++) {
			...
			case 1:
				sum += as_width<1>(index)->mem_used();
				break;
			case 2:
				sum += as_width<2>(index)->mem_used();
				break;
			case 4:
				sum += as_width<4>(index)->mem_used();
				break;
			case 8:
				sum += as_width<8>(index)->mem_used();
				break;
			case 16:
				sum += as_width<16>(index)->mem_used();
				break;
			case 32:
				sum += as_width<32>(index)->mem_used();
				break;
			case 64:
				sum += as_width<64>(index)->mem_used();
				break;
			case 128:
				sum += as_width<128>(index)->mem_used();
				break;
			}
		}
		return sum;
	...
	ptr_t alloc(uint64_t size) {
		...
		case 2:
			return get_slab<2>()->alloc(0);
		case 4:
			return get_slab<4>()->alloc(0);
		case 8:
			return get_slab<8>()->alloc(0);
		case 16:
			return get_slab<16>()->alloc(0);
		case 32:
			return get_slab<32>()->alloc(0);
		case 64:
			return get_slab<64>()->alloc(0);
		case 128:
			return get_slab<128>()->alloc(0);
		default:
			return nullptr;
		}
	}
	...
	void dealloc(ptr_t ptr) {
		...
		switch (slab_widths[index]) {
		case 1:
			as_width<1>(index)->dealloc(ptr);
			break;
		case 2:
			as_width<2>(index)->dealloc(ptr);
			break;
		case 4:
			as_width<4>(index)->dealloc(ptr);
			break;
		case 8:
			as_width<8>(index)->dealloc(ptr);
			break;
		case 16:
			as_width<16>(index)->dealloc(ptr);
			break;
		case 32:
			as_width<32>(index)->dealloc(ptr);
			break;
		case 64:
			as_width<64>(index)->dealloc(ptr);
			break;
		case 128:
