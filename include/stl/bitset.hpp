...
public:
	...
	constexpr bitset(const bool* begin, const bool* end) {
		...
		for (; iterator_index < min((std::size_t)(end - begin), N); iterator_index++) {
			if (begin[iterator_index]) val |= 1 << bit_index;
			bit_index++;
			if (bit_index == 8) {
				m_arr[arr_index++] = val;
				bit_index = 0;
	...
	constexpr bitset(const C& other) : bitset(other.begin(), other.end()) { }
	...
	constexpr void set(int idx, bool value) {
		...
		if (value) m_arr[idx >> 3] |= 1 << (idx & 7);
		else m_arr[idx >> 3] &= ~(1 << (idx & 7));
	}
