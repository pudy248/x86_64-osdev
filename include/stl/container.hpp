...
template <typename C, typename T> concept condition = requires
(C comparator, T obj) {
	{ comparator(obj) } -> std::convertible_to<bool>;
};
template <typename C, typename T> concept comparator = requires
(C comparator, T lhs, T rhs) {
	{ comparator(lhs, rhs) } -> std::convertible_to<int>;
};

template <typename C, typename T> concept container = 
requires(const C c) {
	requires requires(int i) { { c.at(i) } -> std::convertible_to<const T>; };
	{ c.size() } -> std::convertible_to<int>;
	{ *c.begin() } -> std::convertible_to<const T>;
};

template <typename C, typename T> concept container_writeable = 
requires(C c) {
...
public:
	...
	constexpr T& operator[](this Derived& self, int idx) {
		return self.at(idx);
	}
	template <class Derived>
	constexpr T& front(this Derived& self) {
		return self.at(0);
	}
	template <class Derived>
	constexpr T& back(this Derived& self) {
		return self.at(self.size() - 1);
	}

	template <container_writeable<T> C2, class Derived>
	constexpr C2 subspan(this Derived& self, int start) {
		return C2(self.begin() + start, self.end());
	}

	template <class Derived>
	constexpr int find(this const Derived& self, const T& elem) {
		for (int i = 0; i < self.size(); i++) if (self.at(i) == elem) return i;
		return -1;
	}
	template <container<T> C2, class Derived>
	constexpr int find(this const Derived& self, const C2& elems) {
		int index = self.size();
	...
	template <class Derived>
	constexpr bool contains(this const Derived& self, const T& elem) {
		return self.find(elem) > -1;
	}
	template <container<T> C2, class Derived>
	constexpr bool contains(this const Derived& self, const C2& elems) {
		return self.find(elems) > -1;
	}

	template <container<T> C2, class Derived>
	constexpr int contains_which(this const Derived& self, const C2& elems) {
		for (int i = 0; i < elems.size(); i++) {
			if (self.find(elems[i]) >= 0) return i;
		}
		return -1;
	}

	template <container<T> C2, class Derived>
    constexpr bool starts_with(this const Derived& self, const C2 other) {
	...
	template <class Derived>
    constexpr bool starts_with(this const Derived& self, const T t) {
		return self.at(0) == t;
	}
	template <class Derived>
    constexpr bool starts_with(this const Derived& self, const T* ptr, int length) {
		return self.starts_with(span<T>(ptr, ptr + length));
	}

	template <container<T> C2, class Derived>
    constexpr bool ends_with(this const Derived& self, const C2 other) {
		if (self.size() < other.size()) return false;
	...
	template <class Derived>
    constexpr bool ends_with(this const Derived& self, const T t) {
		return self.at(self.size() - 1) == t;
	}
	template <class Derived>
    constexpr bool ends_with(this const Derived& self, const T* ptr, int length) {
	...
	template <container<T> C2, class Derived>
	constexpr bool equals(this const Derived& self, const C2 other) {
		return self.size() == other.size() && self.starts_with(other);
	}
	template <class Derived>
	constexpr bool equals(this const Derived& self, const T* ptr, int length) {
		return self.equals(span<T>(ptr, length));
	}
	template <container<T> C2, class Derived>
	constexpr bool operator==(this const Derived& self, const C2 other) {
...
public:
	...
	constexpr container_wrapper(C& other) : _container(&other), 
