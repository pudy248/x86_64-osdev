...
optional<dhcp_packet> dhcp_get(uint16_t port, uint32_t xid) {
	optional<udp_packet> opt = udp_get(port);
	if (!opt)
		return {};
	dhcp_header* h = (dhcp_header*)opt->b.data_begin;
	if (h->xid != xid) {
		udp_forward(opt);
		return {};
	}
	dhcp_info i = (dhcp_info)opt->i;
	i.header = h;
	return dhcp_packet{i, opt->b};
}

optional<dhcp_packet> dhcp_get(udp_conn_t conn, uint32_t xid) {
	optional<udp_packet> opt = udp_get(conn);
	if (!opt)
		return {};
	dhcp_header* h = (dhcp_header*)opt->b.data_begin;
	if (h->xid != xid) {
...
void dhcp_discover(uint32_t xid, ipv4_t preferred_ip) {
	...
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::MESSAGE_TYPE, cbytespan({DHCP_MTYPE::DISCOVER})}, s);

	if (preferred_ip)
		write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::REQUEST_IP, cbytespan((uint8_t*)&preferred_ip, 4)}, s);
	uint8_t cid[7] = {HTYPE::ETH};
	ranges::copy(span(cid + 1, 6), global_mac.as_bits);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::CLIENT_IDENT, span(cid, 7)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::HOST_NAME, rostring(HOSTNAME)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::VENDOR_IDENT, rostring(VENDOR)}, s);
	...
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::PRL, span(prl, sizeof(prl))}, s);
...
dhcp_lease dhcp_parse_response(dhcp_packet packet, DHCP_MTYPE expected_mtype) {
	...
	tlv_option_t<DHCP_OPT, uint8_t, false> opt = {};
	do {
		opt = read_tlv<DHCP_OPT, uint8_t, false>(s);
		switch (opt.opt) {
		...
		case DHCP_OPT::MESSAGE_TYPE:
			if ((DHCP_MTYPE)opt.value[0] != expected_mtype) {
...
void dhcp_request(uint32_t xid, const dhcp_lease& offer) {
	...
	obinstream<> s{(std::byte*)(dhcp + 1)};
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::MESSAGE_TYPE, span<const DHCP_MTYPE>({DHCP_MTYPE::REQUEST})}, s);
	uint8_t cid[7] = {HTYPE::ETH};
	ranges::copy(span(cid + 1, 6), global_mac.as_bits);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::CLIENT_IDENT, span(cid, 7)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::REQUEST_IP, span<uint8_t>((uint8_t*)&offer.yiaddr, 4)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::DHCP_SERVER, span<uint8_t>((uint8_t*)&offer.server_ident, 4)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::HOST_NAME, rostring(HOSTNAME)}, s);
	write_tlv<DHCP_OPT, uint8_t, false>({DHCP_OPT::VENDOR_IDENT, rostring(VENDOR)}, s);
