...
static string dns_name(ibinstream<>& s) {
	...
	while (uint8_t len = s.read_b<uint8_t>()) {
		if (name.size()) name.append('.');
		span<const uint8_t> frag = s.read_n(len);
...
static dns_answer get_answer(ibinstream<>& s, dns_header* hdr) {
	...
	span<const uint8_t> d{ s.read_n(len) };
...
static void write_question(obinstream<>& s, dns_question q) {
	...
	for (rostring frag : v) {
		s.write(frag.size());
		s.write((uint8_t*)frag.begin(), frag.size());
	}
	s.write_b((uint8_t)0);
...
retry:
	...
	dns->xid = xid;
	udp_send({ npi, b });
	if (DNS_LOG) qprintf<128>("[DNS] <XID %04x> Query: \"%S\" A IN\n", xid, &domain);

	dns_packet p;
	while (1) {
		if (dns_get(xid, p)) { break; }
		if (timepoint::now() - t1 > units::seconds(5.)) goto retry;
	}
	dns_header* dns2 = p.i.header;
	if (DNS_LOG) qprintf<64>("[DNS] <XID %04x> Response: ", xid);
	ibinstream<> is{ (uint8_t*)(dns2 + 1) };
	dns_question q;
	for (int i = 0; i < htons(dns2->n_quest); i++) q = get_question(is, dns2);
	for (int i = 0; i < htons(dns2->n_ans); i++) {
		dns_answer ans = get_answer(is, dns2);
		rostring aname = q.name;
		if (DNS_LOG) qprintf<128>("\"%S\" %I A IN\n", &aname, *(ipv4_t*)ans.data.begin());
		if (ans.type == DNS_TYPE::A && ans.class_code && DNS_CLASS_CODE::IN) {
			kfree(p.b.frame_begin);
			return *(ipv4_t*)ans.data.begin();
		}
	}
	if (DNS_LOG) qprintf<128>("\"%S\" NOT FOUND\n", &domain);
	kfree(p.b.frame_begin);
	return 0;
}
