...
public:
	...
	constexpr auto& operator[](this Derived& self, idx_t v) {
		return *(self + v);
	}
	template <typename Derived>
	constexpr Derived& operator-=(this Derived& self, idx_t v) {
		return self += -v;
	}
	template <typename Derived>
	constexpr Derived operator+(this Derived self, idx_t v) {
...
template <typename I, typename T>
struct std::indirectly_readable_traits<cast_iterator<I, T>> {
public:
	using value_type = T;
};

template <typename I, typename T>
class cast_iterator : public iterator_crtp<cast_iterator<I, T>> {
public:
	using value_type = T;
	...
	cast_iterator(const I& i) : backing(i) {}
	...
	constexpr value_type operator*() const { return (T)*backing; };
...
template <auto V>
struct std::indirectly_readable_traits<value_iterator<V>> {
public:
	using value_type = int;
};
template <auto V>
class value_iterator : public iterator_crtp<value_iterator<V>> {
	using value_type = decltype(V);
	using difference_type = idx_t;

	constexpr value_iterator& operator+=(idx_t) {}
	constexpr idx_t operator-(const value_iterator&) const { return 0; }
	constexpr value_type operator*() const { return V; };
};