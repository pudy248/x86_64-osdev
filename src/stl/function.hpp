...
template <typename... T>
consteval static size_t parameter_pack_count() {
	return sizeof...(T);
}
template <typename... T>
consteval static size_t parameter_pack_size() {
	return (0 + ... + sizeof(T));
}
...
protected:
	array<std::byte, parameter_pack_size<Args...>()> arg_data;
	R (*function)(Args...);
	bool _has_args;

public:
	constexpr function_instance(R (*fn)(Args...)) : function(fn), _has_args(false) {}
	constexpr function_instance(R (*fn)(Args...), Args... args) : function(fn), _has_args(true) {
