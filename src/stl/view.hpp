...
protected:
	...
	constexpr I from(const I& start, const idx_t index) const {
		if constexpr (std::random_access_iterator<I>)
			return start + index;
		else {
			I iter = start;
			for (idx_t i = 0; i < index && iter != m_sentinel; ++i, ++iter)
				;
			return iter;
		}
	}
...
public:
	...
	constexpr view(const I& begin = I{}, const S& end = S{}) : m_begin(begin), m_sentinel(end) {};

	constexpr view(I& begin, S& end) : m_begin(begin), m_sentinel(end) {};

	constexpr view(const I& begin, idx_t length) : m_begin(begin), m_sentinel(begin + length) {};

	template <typename I2, typename S2>
	constexpr view(const view<I2, S2>& other) : m_begin(other.begin()), m_sentinel(other.end()) {}

	template <container C>
	constexpr view(C& c) : view(c.begin(), c.end()) {}
	template <container C>
	constexpr view(const C& c) : view(c.cbegin(), c.cend()) {}
	...
	template <comparable_iter_R<I> R>
	constexpr idx_t find(const R& elem) const {
		I iter = begin();
	...
	template <comparable_iter_R<I> R>
	constexpr idx_t count(const R& elem) const {
		I iter = begin();
		idx_t ctr = 0;
		for (idx_t i = 0; iter != end(); ++i, ++iter)
			if (*iter == elem)
	...
	template <comparable_iter_R<I> R>
	constexpr bool contains(const R& elem) const {
		return find(elem) != -1;
	}
	template <comparable_iter_I<I> I2, typename S2>
	...
	template <comparable_iter_R<I> R>
	constexpr bool starts_with(const R& elem) const {
		return begin() != end() && elem == *begin();
	}
	template <comparable_iter_R<I> R>
	constexpr bool ends_with(const R& elem) const {
		if (Infinite)
			return false;
	...
	constexpr auto& operator[](this Derived self, idx_t index) {
		kassert(DEBUG_ONLY, WARNING, Infinite || index < (idx_t)self.size(), "Out of bounds access to view.");
		return *self.from(self.begin(), index);
	}
	...
	constexpr void fill(R&& val) {
		if constexpr (std::same_as<std::make_unsigned_t<std::remove_cv_t<R>>, uint8_t> || std::same_as<R, std::byte>) {
