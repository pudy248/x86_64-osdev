...
template <iterator_of<uint8_t> I = uint8_t*> class obinstream : public basic_ostream<uint8_t, I> {
public:
	using basic_ostream<uint8_t, I>::basic_ostream;
	using basic_ostream<uint8_t, I>::write;
	void write(const void* ptr, idx_t size) {
		this->write(span<std::iter_value_t<I>>((std::iter_value_t<I>*)ptr, size));
	}
	template <typename T> void write_b(const T& val) { this->write(&val, sizeof(T)); }
};
...
template <iterator_of<uint8_t> I = uint8_t*, std::sentinel_for<I> S = I>
class ibinstream : public basic_istream<uint8_t, I, S> {
public:
	using basic_istream<uint8_t, I, S>::basic_istream;
	template <typename T, container_of<T> C = span<T>> constexpr C read() { return this->read_n(sizeof(T)); }

	template <typename T> constexpr T read_b() { return *(T*)this->read_n(sizeof(T)).begin(); }
};