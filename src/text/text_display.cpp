...
template <typename T> constexpr void set_if_src(T& dest, T src, T inval = (T)-1) {
	if (src != inval) dest = src;
}
template <typename T> constexpr void set_if_not_dest(T& dest, T src, T inval = (T)-1) {
	if (dest == inval) dest = src;
}
...
text_layer::text_layer(span<const int> p, span<const int> d, span<const int> m)
	: buffer(d[0] * d[1])
	, pos(p[0], p[1])
	, dims(d[0], d[1])
...
static constexpr void scroll_up(char* buf, int stride, int rect[4]) {
	...
	for (int x = rect[0]; x < rect[1]; x++) buf[(rect[3] - 1) * stride + x] = ' ';
}
...
text_layer& text_layer::fill(char c, int x1, int x2, int y1, int y2) {
	...
	for (int y = y1; y < y2; y++) {
		for (int x = x1; x < x2; x++) { buffer[y * dims[0] + x] = c; }
	}
	return *this;
}
...
static void newline(text_layer& l, bool right) {
	l.pos[0] = right ? l.margins[1] : l.margins[0];
	l.pos[1]++;
	if (l.pos[1] == l.margins[3] - 1) {
...
static void append_char(text_layer& l, char c, bool never_wrap) {
	l.buffer[l.pos[1] * l.dims[0] + l.pos[0]] = c;
	l.pos[0]++;
	if (l.pos[0] == l.margins[1] && !never_wrap) newline(l, false);
}
...
text_layer& text_layer::display(console& c, bool clear) {
	for (int y = 0; y < min(c.dims[1], dims[1]); y++) {
		for (int x = 0; x < min(c.dims[0], dims[0]); x++) {
			if (buffer[y * dims[0] + x]) c.set(x, y, buffer[y * dims[0] + x]);
			if (clear) buffer[y * dims[0] + x] = 0;
		}
	}
	return *this;
}
text_layer& text_layer::display(text_layer& l, bool clear) {
	for (int y = 0; y < min(l.dims[1], dims[1]); y++) {
		for (int x = 0; x < min(l.dims[0], dims[0]); x++) {
			if (buffer[y * dims[0] + x]) l.buffer[y * l.dims[0] + x] = buffer[y * dims[0] + x];
			if (clear) buffer[y * dims[0] + x] = 0;
		}
	}
	return *this;
...
text_layer& text_layer::hexdump(const void* ptr, uint32_t bytes, uint32_t block_width, uint32_t num_columns,
	...
	if (bytes / block_width % num_columns) newline(*this, false);
	return *this;
}