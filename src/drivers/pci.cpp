...
uint32_t pci_read(pci_addr dev, uint8_t reg) {
	...
	uint32_t address =
		(uint32_t)((lbus << 16) | (lslot << 11) | (lfunc << 8) | (lreg << 2) | 0x80000000U);
...
void pci_print() {
	for (int i = 0; i < globals->pci->numDevs; i++) {
		printf("(%i:%i:%i) dev:%04x vendor:%04x class %02x:%02x:%02x:%02x\n",
			   globals->pci->devices[i].address.bus, globals->pci->devices[i].address.slot,
			   globals->pci->devices[i].address.func, globals->pci->devices[i].device_id,
			   globals->pci->devices[i].vendor_id, globals->pci->devices[i].class_id,
			   globals->pci->devices[i].subclass, globals->pci->devices[i].prog_if,
			   globals->pci->devices[i].rev_id);
	}
}
