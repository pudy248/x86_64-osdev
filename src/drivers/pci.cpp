...
void pci_init() {
	globals->pci = decltype(globals->pci)::make_unique((pci_devices*)walloc(sizeof(pci_devices), 0x10));
	int device_idx = 0;
...
void pci_print() {
	for (int i = 0; i < globals->pci->numDevs; i++) {
		printf("(%i:%i:%i) dev:%04x vendor:%04x class %02x:%02x:%02x:%02x\n", globals->pci->devices[i].address.bus,
			   ...
			   globals->pci->devices[i].class_id, globals->pci->devices[i].subclass, globals->pci->devices[i].prog_if,
